...
```text
https://ijudge.it.kmitl.ac.th/submissions/558105/overview
```
...
```text
โจทย์ มีแม่สีให้ 3 สี แดง เหลือง น้ำเงิน แล้วให้เราทำการหาว่า ถ้านำแม่สี input มาผสมกันจะได้สีอะไร เป็น output แต่ถ้าสีที่นำมาผสมกันไม่ใช้แม่สี แดง เหลือง น้ำเงิน ให้แสดงค่าเป็น Error 
```
...
```text
1. สร้างฟังก์ชัน ชื่อ main สำหรับทำงานหลักของโปรแกรม
2. สร้างตัวแปร 2 ตัวในการเก็บข้อมูล สี 
3. สร้าง if else ถ้านำสีแต่ละสีมาบวกกันจะได้อะไรแล้วปรื้นออกมา
4. ทำกำหนดว่า ถ้าไม่ใช้ สีที่ทำไว้ให้แสดงค่าเป็น error
5. แสดงผล output ตามที่คาดไว้
...
```text
แผนตอนแรกก่อนที่จะให้เพื่อน pair ช่วย   
ตือการให้รับตัวแปรสองตัว โดยผมต้องเป็น firstcolors เพื่อรับเ input สีแรก และ secondcolors เพื่อรับ input ตัวที่สอง ต่อมาผมคิดว่าจะใช้ if else ในการทำ  ถ้าสีแรก เท่ากับ input ตัวอักษร "Red" แล้วสีที่สอง เท่ากับ  input "Yellow" ให้ปริ้นสีส้มออกมา ผมไม่แน่ใจต้องทำยังไงก็ต่อในส่วน testcase ก็เลยสอบถามเพื่อน หลังจากนนี้จะเป็นการทำแผนต่อที่ได้คำแนะนำจากเพื่อน pair สุดท้ายให้จบโดยการที่ใช้ else เพื่อให้ข้อมูลที่ input ใส่มถ้าไม่ใช้ แม่สี ที่เราทำไว้ใน if ให้แสดงผลเป็น error
...
```text
เป็นการพิมพ์ตัวพิมพ์ใหญ่ทั้งหมด เพื่อหาว่าจะแสดงผมได้ไหม
```
...
```text
เป็นการหาว่า พิมพ์เล็ก จะแสดงผลได้เหมือนเดิมหรือไหม
```
...
```text
ถ้าใส่สีที่ได้จากการผสมสีของแม่สี แดงกับเหลือซึ่งคือ ส้ม จะได้ผลเป็นยังไง
```
...
```text
เพื่อน pair week ที่ 3  โต๋(tor)  69070005
```

เขาช่วยอะไร

```text
เขาช่วยผมในการบอกว่า testcase ไม่ได้มีแค่ตามที่หน้าเว็ปบอก ซึ่งคำตอบก็คือ กรณีที่ สีแรกและสีที่สอง คือสีเดียวกัน จะเป็นสีนั้นๆ และผมพบเจอปัญหาในการเขียนโค้ดเพราะหนึ่งบรรทัด ยาวเกินไปเลยหาวิธีแก้ซึ่งก็คือการ ตัด ตรงส่วน  or ลงมา 
```

คุณยังทำอะไรด้วยตนเอง

```text
อ่านโจทย์ วิเคราหะ์ โจทย์ลองทำตามแผนของตัวเองและส่งงาน
```

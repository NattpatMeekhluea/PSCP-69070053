...
```text
https://ijudge.it.kmitl.ac.th/submissions/583020/overview
```
...
```text
โจทย์ให้สร้างโปรแกรมที่จะใส่เลขจำนวณเต็ม  1 ตัว แล้วให้หาว่าจำนวนนั้น ตั้งแต่  int input ถึง 0  มีเลขอะไรบ้างที่หาร ด้วย "10" ลงตัวไม่มีเศษ และแสดงผลเลขเหล่านั้น

input จะเป็น จำนวนจริงเต็มบวก 

Output จะเป็น จำนวนจริงเต็มบวก ที่หารด้วย 10 ลงตัว และจะเรียงจาก int intput ใส่ จนถึง 0  โดยที่มีคั้นตรงกลางด้วยช่องว่าง
```
...
```text
สร้าง int(input) ขึ้นมาเพราะเป็นจำนวณเต็ม 
สร้าง ตัวแปร ที่เป็นตัวกำหนดเป็น เลขที่ไม่เกิด input แรก และหาร 10 ลงตัว ในที่นี้คือ
ตัวแปร2 = ตัวแปร1 ลบด้วย (ตัวแปร1 หารแแบบไม่เอาเศษด้วย 10) [ ตัวแปร % 10 ]
ใช้ ฟั้งชั้น range ในการหา โดยกำหนดให้ starts = ตัวแปร2 stop = -1 step = -10 
ทำการแสดงผลของตัวแปรที่ 2  โดยกำหนดตัวคั้นกลางด้วยช่องว่าง end=" "
```
...
```text
ได้ทำตามแผนแรก การให้ตัวแปรที่ สอง = ตัวแปรแรก ลบด้วย ตัวแปรแรก หารไม่เอาเศษ 10 จะได้เลขที่เราต้องการ ตัวอย่าง 
ตัวแปรแรก = 124
ตัวแปรที่สอง = 124 - (124 %  10)  = 124 -4   = 120 หาร 10 ลงตัว
การใช้ range จะเป็นการหาจุดถึงจุดที่กำหนด นั้นคือ 0 (-1) เพราะฉะนั้น  
จะเริ่มหาใหม่อีกรอบ  120 ไปเรื่อยๆจนถึงจุดกำหนด และ ระหว่างนั้นการแสดงผลจะเพิ่มช่องว่างด้วย end=" "  
```
...
```text
เพราะอยากจะทราบว่าสามารถหา -1 ได้หรือไหม
```
...
```text
เพราะอยากทราบว่า ถ้าเป็นหลักพัน จะแสดงผลยังไง
```

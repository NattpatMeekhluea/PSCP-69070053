...
```text
https://ijudge.it.kmitl.ac.th/submissions/587407/overview
```
...
```text
โจทย์กำหนดให้รับข้อความจำนวณ 1 บรรทัด มีความยาว เริ่มต้นที่ 1 ตัวอักษรแต่จะไม่น้อยกว่า 1000 แล้วทำการนับจำนวณสระ a e i o u ได้ทั้งพิมพ์ใหญ่และเล็ก แล้วทำการแสดงผลตามลำดับ a e i o u ในรูป แบบ สระ : จำนวณ
```
...
```text
สร้างตัวรับรับข้อความและเก็บไว้ในตัวแปร
เปลี่ยนข้อความทั้งหมดให้เป็นตัวพิมพ์เล็กด้วย .lower() เพื่อที่จะอ่านได้ทั้งหมด
สร้าง list สำหรับเก็บจำนวนของสระทั้ง 5 ตัว โดยเริ่มจาก 0
อ่านข้อความทีละตัวอักษร
ถ้าตัวอักษรเป็นสระ ให้เพิ่มจำนวนในตำแหน่งของสระนั้นๆ
แสดงเฉพาะสระที่มีจำนวนมากกว่า 0 ตามลำดับ a e i o u
จัด Output ให้ตรงรูปแบบ อย่าง a : 1
```
...
```text
เป็นไปตามที่แผนแรก 
การใช้ lower ในตัวinput เพื่อให้ทุกตัวเป็นพิมพ์เล็กและนั้นจะทำให้ไว้ว่าจะพิมพ พิมพ์ใหญ่มาก็จะเป็นพิมพ์เล็กและจะอ่านร่วมกันได้

การใช้ลูป เพื่อตรวจว่าใน input มีตัวสระที่กำหนดไว้หรือไม่ และสร้างตัวแปร ตำแหน่ง ที่จะเก็บค่า การเช็คต่ำแหน่งของตัวสระ จากนั้นให้ count ที่เป็นตัวแปร list จำนวนครั้งที่พบสระ ในแต่ละตำแหน่ง ทำการอ่านตัวแปร ต่ำแหน่งแล้ว +1 เข้าไปในต่ำแหน่งที้เก็บได้  

ส่วนอีกลูปจะเป็นการหาว่า ในตัวแปรต่ำแหน่ง ตั้งแต่ 0-4 (5)  ถ้าตัว count ที่อ่านค่า ตัวแปร ต่ำแหน่ง มากกว่า 0 ให้แสดงผลตามที่กำหนด 

print(vowels[position], ":", count[position])
vowels[position] คือ ตัวแปรอักสระ[ในต่ำแหน่ง] 
count[position]) คือ จำนวนตัวอักสระ[ในต่ำแหน่ง
```
...
```text
ถ้าผมพิมพ์ข้อความที่ไม่มีตัวอักษรเลยจะแสดงตามที่กำหนดได้ไหม
```
...
```text
ถ้าพิมพ์เป็นตัวพิมพ์ใหญ่ทั้งหมดจะเปลี่ยนเป็นพิมพ์เล็กแล้วแสดงผลได้เหมือนเดิมไหม
```

Input:

```text
HELLO GUYS I"M SKIBIDI
```
...
```text
ถ้าพิมพ์ตัวอักษรพิเศษระหว่างกันยังอ่านได้ปกติไหม
```

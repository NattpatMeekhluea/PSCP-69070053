...
```text
https://ijudge.it.kmitl.ac.th/submissions/590031/overview
```
...
```text
โจทย์ให้อ่านค่าจำนวณเต็ม สี่ตัว แล้วนำมาหาจำนวณที่สอดคล้องกัน  การหาจะหาจาก ตัวแปรจำนวณเต็ม A และ B 
โดยจะหาในช่วงตัวแปรนี้ ทำแล้วทำการหาร ด้วย ตรวจแปล d  แล้วตรวจสอบเศษของผลลัพทธ์สมการว่าตรงกับ r ไหม

input
จะรับค่าจำนวณเต็ม 4 input 
ตัวที่ 1 และ 2 จะเป็นจำนวณนับ
ตัวที่ 3 จะเป็นจำนวณนับ ที่จะนำมาหารในสมการ
ตัวที่ 4 จะเป็นจำนวณนับที่จะเป็นเศษในการหาสมการ
...
```text
รับค่าจำนวณเต็ม A B  d และ r
สร้างตัวแปรเก็บจำนวณการสอดคล้อง count ให้เท่ากับ 0
ใช้ ลูป โดยการสร้างตัวแปรเก็บข้อมูลลูป นั้นคือ number
สำหรับ number ที่อยู่ใน A ,B + 1
  ถ้า number หาร กับ d แล้ว เท่ากับ r
ตัวเก็บข้อมูลการสอดคล้อง count จะบวก 1
แสดงผล count
...
การใช้ for in range นี้เป็นตามโจทย์ทีี่ต้องการหาว่า เลขระหว่าง Aและ B นำไปหาร กับ d แล้ว  จะได้เท่าไร หากได้เท่ากับ r ที่กำหนดแสดงว่า ข้อมูลการสอดคล้องจะเพิ่ม1 
การเติม +1 ใน [A,B+1] เพราะว่าโจทย์ต้องการหา เลขระหว่าง A , B แต่ใน python หากใส่ แค่ [A,B]  ตัว B  จะไม่ได้ตรงกัน เช่น
A = 20  
B = 30 
[A,B] = 20 ถึง  29
[A,B+1] =  20 ถึง 30
...
```text
ตรวจกรณีที่ไม่ตรงกับเงื่องไขเลย
```

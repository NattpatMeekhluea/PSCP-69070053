...
| OJ submission ID, if submitted | https://ijudge.it.kmitl.ac.th/submissions/583402/overview |
| OJ status | Pass |
...
```text
ผมใช้ AI ช่วยอธิบายวิธีหาคะแนนต่ำสุด ตรวจสอบแผนแรก,ยกตัวอย่างเพื่อทดสอบสูตร อธิบายการใช้ max() เพื่อป้องกันคะแนนติดลบ และช่วยตรวจเงื่อนไขกับโค้ด ส่วนการวิเคราะห์โจทย์ การคำนวณ และการเขียนโค้ด ผมทำและทดสอบด้วยตนเอง
```
...
```text
AI ช่วยให้ผมสักเกตุว่าทำไม testcase ผมถึงผิด และผลปรากฎว่า  ผมไม่ได้สร้างคำสั่งที่จกำหนดให้ค่าของ คะแนนต่ำสุด ไม่น้อยกว่าสูง ซึ่งในที่นี้คือการใช้คำสั่ง max 
```
...
```text
ในต้องแรกผมตรวจสอบว่าทำไม testcase  ถึงผิดหมด จะมีอยู่สองตัว ตัวแรก ผมเขียนตัวแสดงผล Surprising กับ not Surprising ตัว s เป็นพิมพ์เล็กหมดเลยผิด สอง ผมกำหนดให้ input เป็น int ในตอนแรกแต่โจทย์ให้เป็นจำนวน จริง (float) เลยเกิด value error  
```
...
```text
ผมได้เรียนรู้คำสั่ง max ซึ่งคือการกำหนดเลือกเฉพาะค่าที่มากที่สุด ช่วยให้ผมกำหนดว่า ตัวแปรไม่ต่ำกว่า 0 ได้ แต่ส่วนอื่นๆผมย้ังต้องทำเองอยู่ 
```

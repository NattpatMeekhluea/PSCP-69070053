...
```text
https://ijudge.it.kmitl.ac.th/submissions/583402/overview
```
...
```text
โจทย์นี้ ให้ ค่าของคะแนนนั้น ได้ตั้งแต่ 0 ถึง 10 และเป็นจำนวณจริง (float) 
เมื่อรวบรวมเสร็จแล้วจะแสดงเพียงแค่ คะแนนรวม และคะแนนสุดสุด แต่ถ้าคะแนนของคนที่มากที่สุด ห่างกันเกินสองคะแนนกับคนที่คะแนนน้อยที่สุดจะแสดงข้อความความปลาดใจ และจะต้องมีใครมาช่วยดึงคะแนนแน่นๆ (ในนี้คือคะแนนของคนที่ได้อันดับสอง) ถ้าไม่ได้เกินสองจะแสดงข้อความไม่ประหลาดใจ
...
```text
เราต้องสร้าง input สองตัว ที่เป็น float คือส่วนของคะแนนรวม และคะแนนสูงสุด และสร้างตัวแปรคะแนนต่ำที่สุด ที่ได้จากการหาด้วยสมการ 

คะแนนรวม - คะแนนสูงสุด x สอง ลองทดสอบ 

สมการมะกี้ ว่าสามารถหาค่าได้หรือไหม เมื่อทดสอบแล้วได้ให้ สร้างตัวแปรที่ 3 คือตัวแปรของ ค่าความต่าง ระหว่างคะแนนสูงสุดกับคะแนนต่ำสุด และทำใช้ ฟังชั้นกำหนดว่า ตัวแปรที่ 3 หาก มากกว่า 2 ให้แสดงผล ประหลาดใจ อื่นๆ แสดงผลไม่ประหลาดใจ
```
...
```text
เป็นไปตามแผนแรก แต่มีการปรับเปลี่ยน การใช้สมการ คะแนนรวม - คะแนนสูงสุด x สอง เพราะว่าจะเป็นการหาคะแนนต่ำที่สุด การใช้คูณสองเพราะว่าคะแนนที่มาช่วยนั้นคือคะแนนที่สอง เรากำหนดให้สูงเท่าที่เป็นไปได้ เพื่อหาคะแนนคนสุดท้ายที่ต่ำที่สุด 

ตัวอย่าง คะแนนรวม = 20 คะแนน สูงที่สุด = 8 
20 - (8 X2) 
= 20 -16 
= 4 คะแนนต่ำสุดจะอยู่ที่ 4 
แต่มีการเปลี่ยนแปลง จาก คะแนนรวม - คะแนนสูงสุด x สอง จะเป็น max(0,คะแนนรวม - (คะแนนสูงสุด x 2) 
เพราะคำาสั่ง max จะเป็นการเลือกค่าที่มากที่สุด และโจทย์ได้กำหนดว่า คะแนนจะมีตั้งแต่ 0 ถึง 10 เพราะฉะนั้นจะต่ำกว่าศูณย์ไม่ได้ ในกรณที่ผลของสมการได้ค่าน้อยกว่า 0 คำสั้ง max จะเลือก 0 แทน ตัวอย่าง max(0,20-(11*2) 0,20-22 max(0,-2) จะเลือก 0
```
...
```text
เมื่อใส่ 0 ในค่าของ คะแนนสูงสุดจะแสดงผลตามที่ตั้งไว้หรือไหม
```
...
```text
เมื่อใส่ -10 ในค่าของ คะแนนสูงสุดจะแสดงผลตามที่ตั้งไว้หรือไหม
```
...
```text
เมื่อใส่ 15 ในค่าของ คะแนนสูงสุดจะแสดงผลตามที่ตั้งไว้หรือไหม
```
...
```text
ควรไม่แสดงเพราโจทย์บอกว่า 0 ถึง 10  
```

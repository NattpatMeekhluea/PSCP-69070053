...
| OJ submission ID, if submitted | https://ijudge.it.kmitl.ac.th/submissions/585278/overview |
| OJ status | Pass |
...
```text
ผมให้ AI ช่วยริวิว แผนแรกของผมและหาข้อที่สามารถปรับปรุงให้ดีขึ้นได้ และช่วยตรวขสอบการอธิบายโจทย์ว่าเข้าใจจริงหรือป่าว พร้อมกับริวิวโค้ด
```
...
```text
AI ช่วยให้สังเกตุว่า ทำไมผมถึงได้  possibly-used-before-assignment หรือก็คือมีค่าที่ไม่ได้กำหนดแต่ถูกใช้แล้ว ในที่นี้คือ ค่า cel กับ result ผมเลยได้กำหนดให้มันเป็น 0.0 ปัญหาจึงหาย
```
...
```text
แม้โจทย์ไม่ได้กำหนด แต่ผมได้สังเกตุว่าโจทย์ให้ค่าอุณหภูมิ C R  F K เป็นพิมพ์ใหญ่เลยให้กำหนด input 2 กับ 3 เป็น ตัวพิมพ์ใหญ่ด้วย.upper()
...
```text
ผมได้เรียนรู้ว่าต้องดูให้ดีว่า ตัวแปรที่นำมาใช้เราได้ทำการกำหนดมันแล้วหรือยัง ทำให้ผมต้องตรวจสอบโค้ดของตัวให้ให้ดีขึ้น 
```
